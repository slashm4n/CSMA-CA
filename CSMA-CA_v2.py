...
p_packet = 0.35 # the probability of sending a packet
...
class TransmitterNode:
    ...
    def try_send_data(self, t, node_activity):
        SEND_DATA = rn.choice([0, 1], p=[1 - self.p_packet, self.p_packet]) # 1 if we want to send data

        if (not self.channel[t]) and SEND_DATA: # if the channel is idle and we want to send data
...
def run_simulation(timestamps, num_nodes, p_packet, packet_length):
    ...
    for t in range(timestamps):
        for idx, node in enumerate(nodes):
            node.try_send_data(t, node_activities[idx]) # runs each node one after the other
...
class CSMAGui:
    ...
    def draw_static_grid(self):
        for row in range(0, 1 + self.num_nodes):
            ...
            for t in range(0, self.time_slots, 5):
                ...
                self.canvas.create_line(x, 0, x, self.row_height * (1 + self.num_nodes), dash=(2, 4))
                self.canvas.create_text(x + 2, self.row_height * (1 + self.num_nodes),
    ...
    def animate_step(self):
        ...
        for row in range(self.num_nodes):
            busy = self.node_activities[row][t]
