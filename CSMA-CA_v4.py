...
class TransmitterNode:
    ...
    def try_send_data(self, t, send_data, counter, CTS, node_activity):
        ...
        # if the channel is idle, we have sent the RTS and we want to send data
            ...
            if counter == 0: # send the packet
                ...
                self.channel[t] = 1
                counter -= 1
            elif counter <= 3: # wait 3
                CTS = 0
                counter -= 1
        else:
            counter = 0
        return send_data, counter, CTS
...
def run_simulation(timestamps, num_nodes, p_packet, packet_length):
    ...
    CTS = [0] * num_nodes # tells us which node received the CTS
    ...
    for t in range(timestamps):
        ...
        for idx, node in enumerate(nodes):
            ...
            if send_data[idx]:
               print(idx, t)
            send_data[idx], counters[idx], CTS[idx] = node.try_send_data(t, send_data[idx], counters[idx], CTS[idx], node_activities[idx]) # runs each node one after the other
            if CTS[idx]:
                receiver[t] = 1
...
class CSMAGui:
    def __init__(self, root, channel, node_activities, receiver, step_ms=200):
        ...
        self.colors = colors = {0: "white", 1: "light grey", 2: "red", 3: "green", 4: "light blue", 5: "light green", 6: "dark olive green"}
    ...
    def animate_step(self):
        ...
        receiver_busy = self.receiver[t]
        receiver_color = "dark olive green" if receiver_busy else "white"
        self.canvas.itemconfig(self.receiver_rects[t], fill=receiver_color)
