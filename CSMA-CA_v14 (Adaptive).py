...
class QLearningBackoff: # Q-Learning agent for adaptive backoff duration
    ...
    def action_to_backoff(self, action): # sets the backoff duration
    ...
    def get_backoff_duration(self): # get the backoff duration
...
class TransmitterNode:
    ...
    def generate_new_data(self):
        if rn.random() < self.packet_prob:
            ...
            self.rl_agent.update_packet_queue(self.data_queue)
    
    def set_rl_backoff(self): # set backoff using RL agent
...
def run_simulation(num_slots, num_nodes, packet_prob, packet_length):
    ...
    csv.writer(open("data/backoffs_RL_n_" + str(num_nodes) + "_p_" + str(packet_prob) + ".csv", "w", newline="")).writerows(backoffs)
